...
#          Chris Caputo <ccaputo@alt.net> - PostgreSQL adaption
# Date:    August 5, 2008
# Orig:    October 1, 2001
...
die <<USAGE unless $routername && $interface && $startdate && $enddate;

USAGE: $0 <router> <interface> -[##d][##h][##m][##s]
   OR: $0 <router> <interface> <mm/dd/yyyy[+hh:mm[:ss]]> <mm/dd/yyyy[+hh:mm[:ss]]>
USAGE
...
print "$routername $interface Traffic\n";
...
$dbh= DBI->connect("DBI:Pg:dbname=$db;host=$host",$user,$pass);
$range="dtime > to_timestamp($startdate, 'YYYYMMDDHHMISS') AND dtime <= to_timestamp($enddate, 'YYYYMMDDHHMISS')";

$statement="SELECT id FROM interface WHERE name ~ '$interface' AND rid = (SELECT rid FROM router WHERE name ~ '$routername')";
...
foreach $interface (@interfaces) {
  ...
  $statement="SELECT counter, EXTRACT(EPOCH FROM dtime), dtime FROM ifInOctets_".$rid." WHERE $range AND id=$interface ORDER BY dtime"; 
  ($intbytes_in, $maxin, $avgin, $nfin) = &interface_stats($statement);
  $bytesin = int($intbytes_in/1000000 + .5);

  $statement="SELECT counter, EXTRACT(EPOCH FROM dtime), dtime FROM ifOutOctets_".$rid." WHERE $range AND id=$interface ORDER BY dtime"; 
